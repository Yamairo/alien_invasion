...
def check_keydown_events(event, ai_settings, screen, ship, bullets, stats):
    if event.key == pygame.K_ESCAPE:
        ...
        filename2 = "highest_level.txt"
        with open(filename, 'w') as file_object:
            file_object.write(str(stats.high_score))
        with open(filename2, 'w') as file_object:
            file_object.write(str(stats.highest_level))
        pygame.quit()
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_keydown_events(
                event, ai_settings, screen, ship, bullets, stats)
            start_game(
                ai_settings,
                screen,
                stats,
...
def start_new_level(
        ...
        aliens,
        bullets):
    """This will start a new level if there are no aliens left"""
    if len(aliens) == 0:
...
def get_number_rows(ai_settings, ship_height, alien_height, stats):
    ...
    number_rows = int(available_space_y * stats.level *
                      ai_settings.fleet_scale / (3 * alien_height)) + 1
    return number_rows
...
def check_highest_level(stats, sb):
...
def load_score(stats):
    ...
    try:
        ...
        with open(filename2) as file_object:
            score2 = file_object.read()
            stats.highest_level = int(score2)
